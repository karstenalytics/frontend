...
3. **Attribute** - Resolve the context for each classified transaction: when it happened (time), which pool, which token, which wallet
4. **Normalize** - Convert multi-token values to a common base denomination using on-chain swap rates, so figures are comparable across days and tokens
5. **Present** - Aggregate into JSON files per dimension (by token, by pool, by type, etc.), host as static assets, render as interactive Plotly charts

**Processing Model:**

All processing happens offline in a separate backend repository. The frontend displays pre-computed JSON files, so page loads are fast regardless of data volume.
...
- **Treasury accounts** - Where protocol revenue accumulates
...
The core objective is **identifying which instruction is being executed** in each transaction touching the accounts I monitor.
...
- Categorization for grouping related transactions (there might be different instruction types I want to sum up and display as one type)
- Revenue flags indicating which transactions generate value versus those that merely convert or move it (e.g. a token swap distributes existing revenue but does not create new revenue)
...
This means a single fee transaction can show up in a "revenue by token" chart, a "revenue by pool" chart, and a "top wallets" table without any duplication or re-processing.
...
To compare and aggregate transactions involving different tokens, values are normalized to a common base denomination. Which denomination depends on the protocol. For example, a protocol that distributes SOL to stakers normalizes to SOL, while one that collects fees directly in USDC may need no conversion at all.

### Conversion Approach

**On-chain rates only.** I never use external price feeds. When tokens are swapped on-chain, the actual exchange rate provides precise valuation for that moment. If there are multiple swaps per day, the conversion rates are combined into an average daily price.
...
*Example:* If a treasury receives 100 USDC on Day 1 and swaps it for 0.5 SOL on Day 5, Day 1 revenue is recorded as 100 USDC (unconverted). On Day 5, the swap reveals the rate and the converted value is recorded then. Day 1's record remains unchanged, preserving the actual state of the treasury on that day.
...
Not every protocol requires this step. Some collect fees directly in a single token, making conversion unnecessary. Others accumulate diverse tokens (USDC, BONK, cbBTC, etc.) that are periodically swapped to a distribution token, making pending-conversion tracking essential.
...
The pipeline targets 100% attribution accuracy: every token inflow must be accounted for by a classified transaction. Automated checks run during processing, and manual verification tools exist for spot-checking individual days.
