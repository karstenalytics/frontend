        ...
        <>
          ...
          <div style={{ marginTop: '24px' }}>
            ...
            <p style={{ marginBottom: '16px' }}>
              Wallets ranked by the number of distinct days they executed fee-generating transactions{walletRange === '30d'
                  ? ' in the last 30 days.'
                  : walletRange === '7d'
                  ? ' in the last 7 days.'
                  : '.'}
...
- **Identify power users**: Use the Top Wallets by Days Active table to find the most consistently engaged wallets
...
- **Transactions**: Total number of fee-generating transactions across those days
...
Not every wallet whose first observed transaction falls within our data window is genuinely new to the protocol. Some transaction types require prior on-chain state --- for example, you cannot close a position without first having opened one.

When a wallet's first observed transaction is one of these requires-prior-state types, it is classified as a **returning wallet** rather than a new one. This means the wallet interacted with DefiTuna before our data start date and is re-appearing in our window for the first time.
...
- **Purpose**: Identifies consistently engaged wallets rather than high-volume traders
...
- **Data start date**: Our transaction history begins July 24, 2025. Wallets active before this date will appear as "returning" on their first observed transaction, not "new". True lifetime metrics require a complete history from protocol launch.
