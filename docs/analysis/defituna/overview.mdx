...
- Where does DefiTuna's revenue come from? -> [By Pool](./fees-revenue/by-pool), [By Type](./fees-revenue/by-type)
- What are the biggest drivers of DefiTuna's protocol revenue? → [Pools vs. Types](./fees-revenue/pools-vs-types)
- Why is the protocol revenue on certain days higher than on other days? → [Types per Day](./fees-revenue/by-type)
- How big is the impact of Fusion on DefiTuna's revenue? → [Orca vs. Fusion](./fees-revenue/orca-vs-fusion)
- Is the supply tightening because more TUNA is getting staked? Which wallets are staking or withdrawing the most? → [Staking Overview](./staking/staked-tuna)
- What's the reward behaviour of stakers - do they compound or "just" claim? → [Staker Conviction](./staking/staker-conviction)
...
- When will vested TUNA tokens unlock and how much is currently locked? → [Vesting Timeline](./staking/vesting-timeline)
