...
Displays protocol revenue contribution across different tokens, showing which tokens generate treasury inflows. Revenue comes from two sources: "SOL (direct)" represents native SOL/WSOL received directly, while other tokens (USDC, JUP, TUNA, etc.) are converted to SOL via on-chain swaps ("SwapReward"). Click on a token bar to display the top 10 contributing transactions for that token.
...
- **Analyze revenue composition**: Track which tokens contribute to overall treasury revenue
- **Investigate specific tokens**: Click bars to examine individual transactions for any token
- **Compare token contributions**: Evaluate relative contribution across different tokens
...
Each transaction is classified using log-based pattern matching. The [revenue attribution system](/analysis/data-pipeline#token-normalization) achieves 100% accuracy by reconciling two independent accounting methods.
