...
description: "TUNA tokens locked in vesting schedules and unlock timeline"
chart_type: area
---
...
Tracks TUNA tokens locked in vesting schedules and their unlock timeline, showing how locked TUNA decreases over time and how much unlocked TUNA remains staked or held by vesting wallets.
...
<BrowserOnly fallback={<div style={{ minHeight: '120px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><LoadingSpinner /></div>}>
  {() => {
    ...
    function VestingMetrics() {
      ...
      const totalTuna = schedules.reduce((sum, s) => sum + s.locked_tuna, 0);
      ...
      const remainingPctNow = lockedNow != null ? (lockedNow / totalTuna) * 100 : null;
      const remainingPct7dAgo = locked7dAgo != null ? (locked7dAgo / totalTuna) * 100 : null;
      const remainingPctChange = remainingPctNow != null && remainingPct7dAgo != null
        ? remainingPctNow - remainingPct7dAgo
        : null;

      // Vesting wallet conviction: (staked - locked + held) / unlocked
      // Only the freely-available portion of staked counts (locked stake can't be withdrawn).
      // This way unlocks don't mechanically change the ratio - only actual buying/selling does.
      const stakedNow = stakedTimeline[latestDate] || 0;
      const heldNow = heldTimeline[latestDate] || 0;
      const unlockedNow = totalTuna - (lockedNow || 0);
      const freeNow = (stakedNow - (lockedNow || 0)) + heldNow;
      const convictionNow = unlockedNow > 0 ? (freeNow / unlockedNow) * 100 : null;

      const staked7dAgo = stakedTimeline[date7dAgo] || 0;
      const held7dAgo = heldTimeline[date7dAgo] || 0;
      const unlocked7dAgo = totalTuna - (locked7dAgo || 0);
      const free7dAgo = (staked7dAgo - (locked7dAgo || 0)) + held7dAgo;
      const conviction7dAgo = unlocked7dAgo > 0 ? (free7dAgo / unlocked7dAgo) * 100 : null;
      const convictionChange = convictionNow != null && conviction7dAgo != null
        ? convictionNow - conviction7dAgo
        : null;

      const totalSupply = metadata?.total_supply || 1000000000;
      ...
      return (
        <div className="usage-summary-grid">
          <MetricCard
            ...
            decimals={1}
            suffix=" M TUNA"
            tooltip={`Sum of TUNA locked across all vesting schedules.\n${((totalTuna / totalSupply) * 100).toFixed(2)}% of total TUNA supply.`}
          />
          <MetricCard
            ...
            tooltip={`${((lockedNow || 0) / 1e6).toFixed(1)} M TUNA still locked in vesting schedules.\nChange vs. 7 days ago in percentage points.`}
          />
          <MetricCard
            ...
            tooltip={`(Freely available) / unlocked across all vesting wallets.\nFreely available = staked (minus locked portion) + held in wallet.\nMeasures how much of the unlocked TUNA recipients retain at their free disposal.\n>100% means wallets acquired extra TUNA beyond their unlocked amount.\nWhen above 100%, new unlocks mathematically pull the ratio toward 100% even with perfect retention.\nChange vs. 7 days ago in percentage points.`}
          />
        </div>
      );
    }
...
<BrowserOnly fallback={<LoadingSpinner />}>
  {() => {
    ...
    const VestingWalletStakingOperationsTable = require('@site/src/components/Vesting/VestingWalletStakingOperationsTable').default;
    ...
    function VestingTimelinePage() {
      ...
      const { schedules, daily_timeline, staked_by_vesting_wallets_timeline, vesting_wallet_staking_operations } = data;
      ...
      return (
        <>
          {/* Unlock timeline chart with staked+held overlay */}
          {(() => {
            const stakedRaw = staked_by_vesting_wallets_timeline || {};
          ...
          {/* Vesting wallet staking operations table */}
          {/* TODO: Uncomment when ready
          {vesting_wallet_staking_operations && vesting_wallet_staking_operations.length > 0 && (
            <VestingWalletStakingOperationsTable operations={vesting_wallet_staking_operations} />
          )}
          */}
            ...
            return (
              <details style={{ marginBottom: '24px' }} open={selectedDate !== null}>
                <summary style={{ cursor: 'pointer', padding: '12px', background: 'var(--ifm-background-surface-color)', border: '1px solid var(--ifm-toc-border-color)', borderRadius: 'var(--ifm-global-radius)' }}>
                  <strong>All Vesting Schedules ({selectedDate ? filteredSchedules.filter(s => s.locked_tuna > 0).length : schedules.filter(s => s.locked_tuna > 0).length})</strong>
                </summary>
                {selectedDate && (
                  <div style={{
                    marginTop: '16px',
                    ...
                    <table style={{ ...tableStyles.table, minWidth: '900px' }}>
                      <thead>
                        <tr style={tableStyles.headerRow}>
                          ...
                          <th style={tableStyles.headerCell}>Locked (TUNA, desc.)</th>
...
**Summary Cards:**
- **Total Vesting**: Sum of TUNA locked across all vesting schedules (tooltip shows percentage of total TUNA supply)
- **Remaining Locked**: Percentage of total vesting allocation still locked
- **Vesting Wallet Conviction**: Ratio of freely available TUNA (staked minus locked + held) to unlocked TUNA across all vesting wallets. Values above 100% indicate wallets acquired additional TUNA beyond their unlocked vesting amount

**How to Use:**
1. Check the summary cards for total vesting, remaining locked, and vesting wallet conviction
2. Use the unlock chart to see the long-term release curve and the historical staked + held overlay
3. Click a date in the "Next 10 Unlock Days" table to filter the full schedules table to that unlock day
4. Use "Clear Filter" to return to all schedules
...
- **Staked + held overlay** (if available): Shows staked TUNA plus TUNA held in wallets with vesting schedules as a single combined daily timeline derived from on-chain history
...
- **Locked (TUNA, desc.)**: Total amount initially locked in this schedule (sorted descending - highest amounts first)
...
- **First unlock**: Occurs at `start_time + cliff_period + unlock_period`
- **Subsequent unlocks**: Occur every `unlock_period` hours after the first unlock
- **Final unlock**: Last event when all locked tokens are released
...
- **Daily snapshot**: Total TUNA locked across all vesting schedules on each calendar day (UTC)
- **Cumulative view**: Sum of locked tokens from all active schedules on that date
- **Decreasing total**: As unlock events occur, the total locked amount decreases by the unlock rate
- **Current locked**: Today's value represents TUNA still locked and not yet unlocked
...
For each vesting wallet, the pipeline fetches all TUNA token account transactions via RPC to build a daily held-balance timeline. This feeds the "Staked + held" chart overlay and the Vesting Wallet Conviction metric.

**Relationship to Other Views**

This vesting timeline complements:
- **[Staking Overview](/analysis/defituna/staking/staked-tuna)**: Shows treasury TUNA allocation (staked vs. unstaked)
- **[Stakers](/analysis/defituna/adoption/active-stakers)**: Tracks wallets executing staking instructions
