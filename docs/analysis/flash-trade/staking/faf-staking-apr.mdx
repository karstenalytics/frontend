...
description: "Rolling 7-day and 30-day FAF staking APR"
chart_type: line
---
...
- Use a single latest total-staked FAF snapshot across all dates (the FAF price varies by date)
...
- No reward distributions occurred from April to late June 2025 (pre-reward sharing period), so the chart starts near zero for that period
...
- APR reflects only USDC reward yield and does not include gains or losses from FAF token price changes
