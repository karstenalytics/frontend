...
- **New %** (orange, right axis): Shown in Daily/Weekly when **Unique** view is selected (not shown in **By Pool** view). Percentage of active wallets whose first-ever platform trade falls within the period. The first 7 days are trimmed because the seen-wallet pool is too small for meaningful percentages.
