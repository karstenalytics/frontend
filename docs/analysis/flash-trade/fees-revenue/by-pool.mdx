...
- **Explore fee composition**: Use the metric toggles to compare Fees (trade fees + position fees like margin, borrow, and funding) and Revenue (protocol fee distributions)
...
- **Metric toggles**: Switch between Fees (trade fees + position fees like margin, borrow, and funding) and Revenue (MoveProtocolFees distributions). The y-axis auto-scales to the active metric.
...
- **Pool and custody accounts**: On-chain transactions from Flash.Trade's pool program accounts and custody accounts (~70+ addresses), which emit fee events during trading operations
- **Fee attribution**: Internal fee events (SwapFeeInternal boundaries, MoveProtocolFees distributions) decoded from transaction logs
...
- Fees metric includes trade fees and position fees (margin, borrow, funding); LP fees are excluded (less than 1% of total)
