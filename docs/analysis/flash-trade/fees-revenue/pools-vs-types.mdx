...
<BrowserOnly fallback={<div style={{ minHeight: '400px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><LoadingSpinner /></div>}>
  {() => {
    ...
    function TopTransactionsTable() {
      ...
      // Compute color maps from transaction data
      const allTx = topTransactionsPool?.transactions || [];
      const poolTotals = {};
      const typeTotals = {};
      for (const tx of allTx) {
        const pn = consolidatePoolName(tx.pool);
        poolTotals[pn] = (poolTotals[pn] || 0) + tx.fee_usdc;
        typeTotals[tx.type] = (typeTotals[tx.type] || 0) + tx.fee_usdc;
      }
      const rankedPools = Object.entries(poolTotals).sort((a, b) => b[1] - a[1]).map(([n]) => n);
      const rankedTypes = Object.entries(typeTotals).sort((a, b) => b[1] - a[1]).map(([n]) => n);
      const poolColorMap = buildColorMap(rankedPools);
      const typeColorMap = buildColorMap(rankedTypes);

      const filterChips = [];
      if (selectedPool) filterChips.push({ label: selectedPool, color: poolColorMap[selectedPool] || '#888888' });
              ...
              <table style={tableStyles.table}>
                ...
                <tbody>
                  {displayTransactions.length > 0 ? displayTransactions.map((tx, idx) => {
                    ...
                    const poolColor = poolColorMap[poolName] || '#888888';
