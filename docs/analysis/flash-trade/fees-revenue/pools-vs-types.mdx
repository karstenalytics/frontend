...
Two-dimensional fee distribution showing how transaction types and liquidity pools interact. The Marimekko chart reveals which pool-type combinations drive fees, where transaction types concentrate, and how fee composition varies across pools. Click any segment to display up to 10 contributing transactions for that pool-type combination.
...
<BrowserOnly fallback={<div style={{ minHeight: '400px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><LoadingSpinner /></div>}>
  {() => {
    ...
    const { TYPE_COLORS } = require('@site/src/components/FlashTrade/TypeStackedAreaChart');
      ...
      return (
        <div>
          <p style={{ marginBottom: '16px' }}>
            ...
            By default, it shows all transactions across all pool-type combinations, sorted by USDC amount.
            Click any segment in the Marimekko chart above to filter by a specific pool-type combination.
          </p>
          ...
          {(selectedPool || selectedType) && (
            <div style={{
              ...
              <span style={{ fontSize: '14px', color: 'var(--ifm-color-secondary)' }}>
                Filtered by:
                {selectedPool && (
                  <strong style={{ color: POOL_COLORS[selectedPool] || 'var(--ifm-font-color-base)', marginLeft: '4px' }}>
                    {selectedPool}
                  </strong>
                )}
                {selectedPool && selectedType && (
                ...
                {selectedType && (
                  <strong style={{ color: TYPE_COLORS[selectedType] || 'var(--ifm-font-color-base)' }}>
                    {selectedType}
                  </strong>
                )}
              </span>
              ...
              <table style={tableStyles.table}>
                ...
                <tbody>
                  {displayTransactions.length > 0 ? displayTransactions.map((tx, idx) => {
                    ...
                    const poolColor = POOL_COLORS[poolName] || '#888888';
                    const typeColor = TYPE_COLORS[tx.type] || '#888888';
                    return (
