...
Data Quality measures are constantly improving, below is a brief summary of what is in place for DefiTuna.
...
- Ensures Treasury token swap correctly attribute to earlier token accumulations
...
- **Causes**: Treasury token swap attribution errors, ledger persistence issues, missing token conversions
- **Resolution**: Review ledger carryforward and Treasury token swap attribution logic

**Over-Attribution**
- **Symptom**: Type attribution exceeds raw tracking (triggers ERROR log)
- **Causes**: Double-counting Treasury token swap transactions, including non-revenue transactions, incorrect attribution logic
