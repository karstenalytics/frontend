...
description: DefiTuna collects fees in dozens of different tokens across two AMMs. I traced every lamport to show how they all end up as SOL in your staking rewards.
authors:
  - name: by Karsten
...
- Everything in this article comes from observing on-chain state and transaction data. I have no access to DefiTuna's source code and may have gotten details wrong, but this is what I pieced together from tracing the fee flow on-chain.
- The mechanisms described below reflect the current on-chain implementation as of March 2026. The original investigation was done end of 2025, but on-chain mechanics are still the same.
...
I wanted to verify that all fees are properly transformed into staker revenue, with nothing lost along the way. After tracing every transaction in my dataset, I can confirm: the pipeline is perfectly accurate. This article walks through how it works.
...
Let's trace a single fee with round numbers. A user deposits 10 SOL as collateral and borrows 20 SOL to open a 3x leveraged LP position on the SOL-USDC Orca whirlpool. DefiTuna charges a protocol fee on the total position value (30 SOL). At the most common rate of 10 bps (0.10%), that is a fee of **0.03 SOL**. Because this is a two-token pool, the fee arrives at the treasury split across both tokens: roughly 0.015 SOL and 1.50 USDC.

The 0.015 SOL is ready for distribution to stakers immediately; it is already in the right denomination. The 1.50 USDC sits in the treasury's USDC token account, accumulating alongside USDC from dozens of other transactions, until a `swap_reward` instruction converts the batch to SOL. Three hours later, a `deposit_reward` instruction deposits all accumulated SOL into the [staking contract](https://solscan.io/account/tUnst2Y2sbmgSgARBpSBZhqPzpoy2iUsdCwb5ToYVJa), where it becomes claimable by TUNA stakers proportional to their stake.
...
**100% of treasury revenue goes to stakers.** There is no on-chain split like Flash.Trade's 50/50 between stakers and protocol treasury. The DefiTuna team benefits through owning approximately 50% of the TUNA supply, so they earn their share through the same staking mechanism as everyone else.
...
An important detail: [Fusion AMM](https://docs.defituna.com/dive-into-defituna/trade/fusion-amm-mechanics-explained) is not a third-party AMM. It is built by the DefiTuna team, a hybrid CLMM + on-chain orderbook that launched in July 2025. DefiTuna and Fusion AMM use separate branding because they serve different roles: Fusion AMM is the underlying trading venue, DefiTuna is the lending and leverage layer on top. But they share the same team, and all revenue from both flows to TUNA stakers.

According to [DefiTuna's documentation](https://docs.defituna.com/dive-into-defituna/provide-liquidity/platform-info/fees), the protocol charges a **protocol fee** on position management (opening, compounding, trigger order execution) and a **liquidation fee** (10% of remaining funds after debt repayment) on liquidated positions. The protocol fee rate varies by pool; the docs use 0.05% as an example, but from analyzing 240,000+ treasury transactions I found the most common tier is 10 bps (0.10%), with other pools at 5 or 3 bps.
...
The conversions are part of the same keeper bot cycle as `deposit_reward`. Every ~3 hours, the bot converts whatever non-SOL tokens have accumulated -- down to dust amounts -- then decides whether to deposit based on the total WSOL balance (see [below](#every-3-hours-like-clockwork)). USDC appears in 96% of conversion cycles because it accumulates fast enough to have a balance every 3 hours. Niche tokens like cbBTC (present in only 6% of cycles) may go days between conversions, not because the bot waits for a minimum balance, but because it takes that long for any fees in that token to appear.
...
This achieves 100% attribution accuracy across the full dataset: zero SOL unattributed, verified daily.
...
I analyzed all 1,297 `deposit_reward` events in my dataset to understand the cadence and determine what triggers a cycle.
...
**The trigger threshold is 1.0 SOL.** When revenue is low, cycles get skipped. By tracking WSOL inflows to the treasury between consecutive deposits, I found a sharp cutoff: across all 1,297 events, not a single normal deposit fired with less than 1.0 SOL accumulated. The smallest deposits barely cleared the line:

| Event date | Gap | WSOL collected |
|------------|-----|---------------|
| 2025-11-27 | 3.0h | 1.005 SOL |
| 2026-02-10 | 6.0h | 1.003 SOL |
| 2026-01-27 | 6.0h | 1.006 SOL |
| 2025-09-07 | 3.0h | 1.008 SOL |
...
**During low-volume periods, skipped cycles become common.** In February 2026, only 43% of cycles had enough revenue to trigger immediately. Over the full dataset:

| Gap | Count | Share | Meaning |
|-----|-------|-------|---------|
| ~3 hours | 983 | 75.8% | >= 1 SOL available, deposit fires |
| ~6 hours | 227 | 17.5% | One cycle skipped |
| ~9 hours | 53 | 4.1% | Two cycles skipped |
| ~12+ hours | 34 | 2.6% | Three or more cycles skipped |
...
Now that the mechanics are clear, let's look at what actually flowed through this pipeline. Over the 232 days in my dataset (July 24, 2025 to March 12, 2026), the treasury collected **8,376 SOL**.

### Revenue by Transaction Type

| Source | Revenue Type | SOL | Share | What generates it |
|--------|--------------|-----|-------|-------------------|
| Orca | Leveraged LP | 4,266 | 50.9% | Position opens, closes, liquidations, compounding |
| Orca | Direct transfers | 146 | 1.7% | Revenue collected before the current fee system was in place |
| Fusion | AMM trading fees | 2,546 | 30.4% | `collect_protocol_fees` sweeps from Fusion pool accounts |
| Fusion | Leveraged LP | 1,334 | 15.9% | Position opens, closes, liquidations, compounding |
...
|------|--------------|-------|
| Fusion SOL-USDC | 3,052 | 36.4% |
| Orca SOL-USDC | 2,237 | 26.7% |
| Fusion USDC-WhiteWhale | 462 | 5.5% |
| Orca SOL-CBBTC | 344 | 4.1% |
| Orca SOL-FARTCOIN | 303 | 3.6% |
| Other (47 pools) | 1,978 | 23.6% |
...
The treasury receives fees in more than 30 different token mints: 57% as direct SOL (ready immediately), 24% as USDC, and the remaining 19% spread across FARTCOIN, WhiteWhale, cbBTC, TUNA, ORE, BONK, and many others. Everything except SOL must pass through the `swap_reward` conversion pipeline before stakers see it.
...
The DefiTuna team is actively working to change this. They plan to wind down support for Orca pools and consolidate all activity on Fusion AMM, their own infrastructure. That will mean a short-term revenue hit; Orca LP currently accounts for over 50% of total revenue. But combined with permissionless pool creation on Fusion, which would allow anyone to launch new trading pairs, the move could fundamentally shift the revenue mix. More pools means more organic trading volume and more `collect_protocol_fees` revenue, a fee source that scales with activity rather than volatility. If the transition succeeds, DefiTuna would move from depending on liquidation spikes to earning a steady base of trading fees on infrastructure they fully control.

But regardless of where the fees come from or how the mix shifts, the pipeline stays the same: fees arrive in dozens of tokens from LPing or trading on Fusion, the keeper bot converts and deposits every three hours, and every lamport reaches $TUNA stakers. That is what I set out to verify, and it holds across the full history since revenue sharing is enabled. The machinery works. What flows through it is the part that is changing.
...
A concrete 3-hour `deposit_reward` cycle traced end-to-end. All amounts are verifiable on-chain.

**The cycle**: Between 03:58 and 06:59 UTC, the treasury processed 74 revenue-generating transactions across multiple pools and token types. At the end of the cycle, a burst of `swap_reward` conversions and a `deposit_reward` deposited the accumulated SOL into the staking contract.
...
|------------|-----------------|----------------|--------------------------|
| 04:11 | Open LP position (Fusion) | -- | -- (26.61 SOL to pool, fee embedded) |
| 05:10 | Open position (Orca) | 3.38 SOL | 102.24 USDC |
| 05:53 | Add liquidity (Tuna) | 6.10 SOL | -- |
| 06:08 | **Liquidate LP (Orca)** | **0.033 SOL** | **0.003 JUP** |
| 06:12 | Open position (Orca) | 11.11 SOL | 8,335 JUP |
| 06:27 | Open position (Orca) | 2.90 SOL | -- |

The liquidation at 06:08 ([View on Solscan](https://solscan.io/tx/26XVasePB1JLqUqLuYCwnhxPoQYpDTrEXuZ561VRx2bdk9rxTUjBWCELA85n3VhkFP5V1uTGUsGF3aWMWBADS1bB)) was a take-profit order execution on a SOL-JUP Orca whirlpool position. The on-chain logs show the full lifecycle in a single transaction: liquidity withdrawal, fee collection, debt repayment to the lending pool (6,716 JUP + 12.38 SOL), and protocol fee transfer to the treasury.
...
| 06:59:03 | [swap_reward_orca](https://solscan.io/tx/4CFZKaxZ26C4yVU13KeXeDBPxuU3piftJLvk35DSa7bi1tCEJEWtHF5EaC5QkLetBdaFUQxVGwjbe7vJH7Kn1u1k) | 0.004 ZEC | 3.064 SOL |
...
```
total_unclaimed_reward: 1109350827451
acc_reward_per_share: 2625355448996
```

That is 1,109.35 SOL in total unclaimed rewards across all stakers. The previous `deposit_reward` (at 03:57 UTC) logged 1,104.55 SOL, so this cycle added **4.80 SOL** to the staking pool. The 4.97 SOL collected during the cycle minus 4.80 SOL deposited leaves a 0.17 SOL difference: tokens that had not yet been converted via `swap_reward` at the moment the deposit fired.

**Where the 4.97 SOL came from (this cycle)**

| Source | SOL |
|--------|-----|
| Direct WSOL from position mgmt (opens, adds, compounds) | 0.15 |
| Direct WSOL from liquidation | 0.03 |
| Direct WSOL from fee collection | 0.08 |
| `swap_reward` conversions (USDC, TUNA, cbBTC, JUP, ZEC) | 4.71 |
| **Total** | **4.97** |

In this cycle, 95% of the SOL that reached stakers started as non-SOL tokens and had to be converted first.

</details>

---

*Revenue data covers July 24, 2025 through March 12, 2026 (232 days). The live version of this data is on the [DefiTuna dashboards](/analysis/defituna/overview), updated daily.*
